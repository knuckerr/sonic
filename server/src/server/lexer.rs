...
pub fn lexer_command(command: &str) -> Command {
    ...
    match parts.as_slice() {
        ...
        ["set", key_value @ ..] => {
            ...
            if let Ok((key, option, value)) = lexer_set_command(&set_command) {
                if option > 0 {
...
pub fn lexer_set_command(command_str: &str) -> Result<(String, u64, Vec<u8>), String> {
...
#[test]
pub fn test_lexer_set_json() {
    let (_, option, value_str) = lexer_set_command("key {\"hello\": \"world\"} ex 12").unwrap();
    assert_eq!(12, option);
    assert_eq!("{\"hello\": \"world\"}".as_bytes(), value_str);
}

#[test]
pub fn test_lexer_set_multi_spaces() {
    let (_, option, value_str) =
        lexer_set_command("key avavava ex vava ex vavava vava ex 12").unwrap();
    assert_eq!(12, option);
    assert_eq!("avavava ex vava ex vavava vava".as_bytes(), value_str);
}

#[test]
pub fn test_lexer_set_multi_ex() {
    let (_, option, value_str) = lexer_set_command("key \"avavava ex vavava\" ex 129292").unwrap();
    assert_eq!(129292, option);
    assert_eq!("\"avavava ex vavava\"".as_bytes(), value_str);
}

#[test]
pub fn test_lexer_set_alone_ex() {
    let (_, option, value_str) = lexer_set_command("key \"ex\"").unwrap();
    assert_eq!(0, option);
    assert_eq!("\"ex\"".as_bytes(), value_str);
}

#[test]
pub fn test_lexer_set_quotes_ex() {
    let (_, option, value_str) =
        lexer_set_command("key \"heue jsjslaoxnsjex jsjsex sjsjs ex\"").unwrap();
    assert_eq!(0, option);
    assert_eq!(
